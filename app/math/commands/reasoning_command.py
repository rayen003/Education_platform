...
class MathGenerateReasoningCommand:
    ...
    def _regenerate_step_with_limit(self, step_index: int, step: str, verification_result: Dict[str, Any], confidence: float) -> str:
        ...
        Args:
            step_index: Index of the step to regenerate
            step: The original step content
            verification_result: Verification result from the previous attempt
            confidence: Confidence score from the previous verification
            
        Returns:
            The regenerated step
        """
        try:
            # Generate new step based on verification feedback
            new_step = self._generate_step_with_feedback(step_index, step, verification_result, confidence)
            return new_step
        except Exception as e:
